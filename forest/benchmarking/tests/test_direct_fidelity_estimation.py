from forest.benchmarking.direct_fidelity_estimation import generate_exhaustive_state_dfe, generate_exhaustive_process_dfe, \
    generate_monte_carlo_state_dfe, generate_monte_carlo_process_dfe
...
def test_exhaustive_state_dfe(benchmarker: BenchmarkConnection):
    texpt = generate_exhaustive_state_dfe(program=Program(X(0), X(1)), qubits=[0, 1],
                                 benchmarker=benchmarker)
    assert len(texpt.experiments) == 2 ** 2 - 1


def test_mc_state_dfe(benchmarker: BenchmarkConnection):
    texpt = generate_monte_carlo_state_dfe(program=Program(X(0), X(1), X(2), X(3), X(4), X(5), X(6)),
                         qubits=[0, 1, 2, 3, 4, 5, 6],
                         n_terms=50, benchmarker=benchmarker)
    assert len(texpt.experiments) == 50


def test_exhaustive_dfe(benchmarker: BenchmarkConnection):
    texpt = generate_exhaustive_process_dfe(program=Program(Z(0)), qubits=[0], benchmarker=benchmarker)
    assert len(texpt.experiments) == 7 ** 1 - 1


def test_exhaustive_process_dfe_run(benchmarker: BenchmarkConnection):
    wfnsim = NumpyWavefunctionSimulator(n_qubits=1)
    process = Program(Z(0))
    texpt = generate_exhaustive_process_dfe(program=process, qubits=[0], benchmarker=benchmarker)
    for setting in texpt.experiments:
        setting = setting[0]
        prog = Program()
...
def test_exhaustive_state_dfe_run(benchmarker: BenchmarkConnection):
    wfnsim = NumpyWavefunctionSimulator(n_qubits=1)
    process = Program(X(0))
    texpt = generate_exhaustive_state_dfe(program=process, qubits=[0], benchmarker=benchmarker)
    for setting in texpt.experiments:
        setting = setting[0]
        prog = Program()
        for oneq_state in setting.in_state.states:
...
def test_monte_carlo_process_dfe(benchmarker: BenchmarkConnection):
    ...
    texpt = generate_monte_carlo_process_dfe(program=process, qubits=[0, 1], n_terms=10,
                                    benchmarker=benchmarker)
    assert len(texpt.experiments) == 10
...
def test_monte_carlo_state_dfe(benchmarker: BenchmarkConnection):
    ...
    texpt = generate_monte_carlo_state_dfe(program=process, qubits=[0, 1], n_terms=10,
                                    benchmarker=benchmarker)
    assert len(texpt.experiments) == 10
